...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    title: `${article.title} | WordPress.velar+ Blog`,
...
export default async function EnBlogArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const pluginMap = new Map(enPlugins.map((p) => [p.slug, p]));
        ...
        <div className="bg-white border-b border-slate-200 py-12 lg:py-16">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-xs text-slate-400">⏱ {article.read_time} read</span>
            ...
            <div className="bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 border border-blue-200/70 rounded-2xl p-5 sm:p-6 flex items-start gap-4">
              ...
              <div>
                <div className="text-xs font-bold text-blue-800 uppercase tracking-wider mb-1">Core Strategic Advantage</div>
            ...
            <aside className="lg:col-span-4 hidden lg:block">
              <div className="sticky top-24 space-y-6">
                ...
                <div className="bg-gradient-to-br from-slate-900 to-indigo-950 rounded-2xl p-6 text-white shadow-md">
                  <div className="text-xs font-semibold uppercase tracking-wider text-blue-400 mb-2">Global Extensions</div>
                  <h4 className="text-lg font-bold mb-2">20+ Production Plugins</h4>
                  <p className="text-xs text-slate-300 leading-relaxed mb-4">
                    One-time payment licenses with lifetime updates and zero recurring subscription fees.
                  </p>
                  <Link
                    href="/en/catalog"
                  ...
                  <section key={idx} id={sectionId} className="scroll-mt-24">
                                ...
                                <div>
                                  <div className="flex items-center justify-between gap-2 mb-2">
                                    <span className="px-3 py-1 text-xs font-bold rounded-full bg-blue-100 text-blue-700">
                                      {pData ? pData.platform : 'Ecommerce'}
                                    </span>
                                    {pData && (
                                      <span className="text-lg font-extrabold text-slate-900">
                                        {pData.price_rub.toLocaleString('en-US')} ₽
                                      </span>
                                    )}
                                  </div>
                    ...
                    {section.type === 'cta' && (
                      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-3xl p-8 sm:p-10 text-white my-8 shadow-xl">
                        ...
                        <Link
                          href={section.content.button_url || '/en/catalog'}
        ...
        <section className="bg-white border-t border-slate-200 py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900">You Might Also Be Interested In</h2>
              <Link href="/en/blog" className="text-blue-600 font-semibold hover:underline text-sm sm:text-base">
                View All Guides →
              </Link>
            </div>
